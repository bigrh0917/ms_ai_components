        ...
        async for session in db_client.get_session():
            ...
            try:
                ...
                with warnings.catch_warnings():
                    ...
                    try:
                        # 临时重定向 stderr 以捕获 passlib 的内部错误输出
...
async def main():
    ...
    # 在这里修改要创建的用户信息
    # ==========================================
    username = "test_user_2"      # 用户名
    email = "test_user_2@example.com"  # 邮箱
    password = "test"            # 密码（明文，会自动加密）
    role = UserRole.USER               # 用户角色：UserRole.USER 或 UserRole.ADMIN
    org_tags = None                    # 组织标签（逗号分隔，如："DEPT_A,DEPT_B"）
    primary_org = None                 # 主组织标签（如："DEPT_A"）
    create_private_org = True          # 是否创建私人组织标签
