...
# 允许 search_service 输出 INFO 级别的日志，用于调试检索问题
...
# 允许 Elasticsearch 客户端输出 ERROR 级别的日志，用于调试索引创建问题
...
# 配置日志输出到控制台（用于调试）
import sys
handler = logging.StreamHandler(sys.stderr)  # 使用 stderr，避免干扰 stdout 的 AI 回复
...
async def test_archive_conversation(db: AsyncSession, user: User, conversation_id: str):
    ...
    try:
        ...
        # 4.7 验证get_conversation_history可以从MySQL读取
...
async def test_readonly_after_archive(db: AsyncSession, user: User, conversation_id: str):
    ...
    try:
        ...
        # 5.3 尝试通过chat_service处理消息（应该返回错误提示）
...
async def main():
    ...
    try:
        # 初始化数据库连接（同步方法，不需要await）
        db_client.connect()
        # 初始化Redis连接（异步方法，需要await）
        await redis_client.connect()
        # 初始化Elasticsearch连接（异步方法，需要await）
